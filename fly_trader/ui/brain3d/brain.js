...
function recolor(inst, data) {
  const meta = inst.geom.meta, n = meta.n, col = inst.colAttr.array, pop = inst.pop, hcol = inst.hcolAttr.array, hstr = inst.hstrAttr.array;
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!inst.activity) { r = b[0] * REST; g = b[1] * REST; bl = b[2] * REST; }
      ...
      const v = inst.activity[i] / 127, mag = Math.abs(v), m = Math.pow(mag, 0.7), h = v >= 0 ? WARM : COOL;
      r = b[0] * DIM * (1 - m) + h[0] * m; g = b[1] * DIM * (1 - m) + h[1] * m; bl = b[2] * DIM * (1 - m) + h[2] * m;
      if (mag > GLOW_MIN) {                                       // the glow: strength 0..1 above the threshold, a hot core toward white
  ...
  inst.halo.visible = glowing > 0; inst.minute = data.minute;
}
